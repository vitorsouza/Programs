...
public class PhotoRenamer5 {
	...
	private static final DateFormat dfIn = new SimpleDateFormat("yyyy-MM-dd HH.mm.ss");
	...
	private static void extractDates(File photoFile) throws Exception {
		Date originalDate = dfIn.parse(photoFile.getName());
		...
		try (Scanner scanner = new Scanner(System.in)) {
			while (true) {
				...
				for (int i = 0; i < photoFiles.size(); i++) {
					...
					builder.append('\t').append(photoName).append(((referenceYear == photoYear) ? "   " : " Y!")).append('\n');
				}

				// Prints the prompt and reads the command.
				System.out.println(builder.toString());
				System.out.println("[Q = Quit, # = Fix photo number #, +# = advance all photos plus # hours, %# = ditto, for minutes, N = Fix all nulls, Y = Fix different years, R = Rename the files and quits.]");
				System.out.print("> ");
